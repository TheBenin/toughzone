package org.flixel
{
	...
	public class FlxPreloader extends MovieClip
	{
		...
		protected var imgTnk:Bitmap 		= new Images.ImgPlayer1();
		protected var imgTnkCannon1:Bitmap 	= new Images.ImgPlayerCannon1();
		...
		protected /*private*/function onEnterFrame(event:Event):void
        {
			if(!_init)
			{
		...
		protected function create():void
		{
			...
			if (CONFIG::USE_FOG_API)
				_bmpBar.y = _height - 35;
			else
				_bmpBar.y = _height-11;
			_buffer.addChild(_bmpBar);
			_buffer.addChild(imgTnk);
			_buffer.addChild(imgTnkCannon1);
			_text = new TextField();
		...
		{
			//	o preloader deste cliente (FOG) fica bem no meio, entao e´ melhor fazer a logo da Flixel andar
			//para que ela apareca bem
			if (CONFIG::USE_FOG_API)
				_logo.x-=0.5;
			//	barra de progresso
			_bmpBar.scaleX = Percent * (_width - 8);
			_text.text = "FLX v"+FlxG.LIBRARY_MAJOR_VERSION+"."+FlxG.LIBRARY_MINOR_VERSION+". Tough Zone "+FlxU.floor(Percent*100)+"% loaded";
			_text.setTextFormat(_text.defaultTextFormat);
			//	tanque seguindo a barra de progresso
			if(Percent > 0.9)
			{
				imgTnk.alpha = 0;
				imgTnkCannon1.alpha = 0;
			}
			imgTnk.y 		= _bmpBar.y - imgTnk.height/2 + _bmpBar.height/2;
			imgTnk.x 		= Percent * (_width - 8) - imgTnk.width;
			imgTnkCannon1.y	= imgTnk.y + imgTnk.height / 2 - imgTnkCannon1.height / 2;
			imgTnkCannon1.x	= imgTnk.x + 0.2*imgTnk.width;
