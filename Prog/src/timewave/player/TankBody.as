package timewave.player 
{
	 ...
	 * @author Wolff
	 */
	public class TankBody extends FlxSprite
	{
		...
		public function TankBody(posX:Number = 0,posY:Number = 0) 
		{
			this.loadGraphic(Images.ImgPlayer1, false);
			this.health = 3;
			this.x = posX;
			this.y = posY;
		...
		public function powerUp(type:uint,object:PowerUpObject):void {
			switch(type) {
				case PowerUpObject.LIFE_UP:
				(TopLevel.actualState as Playstate).player.health = 5;
				if (object.pointsGive) return;
				object.pointsGive = true;
				...
				this.tankDamage = 2;
				(TopLevel.actualState as Playstate).player.turret.loadGraphic(Images.ImgPlayerCannon1Buff);
				(TopLevel.actualState as Playstate).player.turret.origin.x = 18;
				(TopLevel.actualState as Playstate).player.turret.origin.y = 13;
				if (object.pointsGive) return;
				object.pointsGive = true;
				TopLevel.score += ScoreTable.POWERUP_SCORE;
				PointBurst.createBigText("+" + String(ScoreTable.POWERUP_SCORE), this.x, this.y);
		...
		private function pressDown():Boolean {
			return (FlxG.keys.pressed("DOWN") && !FlxG.keys.pressed("UP") || FlxG.keys.pressed("S") && !FlxG.keys.pressed("W") && TopLevel.actualState is Playstate);
		}
		
		private function pressUp():Boolean {
			return (FlxG.keys.pressed("UP") && !FlxG.keys.pressed("DOWN") || FlxG.keys.pressed("W") && !FlxG.keys.pressed("S") && TopLevel.actualState is Playstate);
		}
		
		private function pressLeft():Boolean {
			return (FlxG.keys.pressed("LEFT") && !FlxG.keys.pressed("RIGHT") || FlxG.keys.pressed("A") && !FlxG.keys.pressed("D") && TopLevel.actualState is Playstate);
		}
		
		private function pressRight():Boolean {
			return (FlxG.keys.pressed("RIGHT") && !FlxG.keys.pressed("LEFT") || FlxG.keys.pressed("D") && !FlxG.keys.pressed("A") && TopLevel.actualState is Playstate);
		}
	}
